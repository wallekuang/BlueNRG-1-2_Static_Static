...
uint8_t Add_Chat_Service(void)
{
  ...
  */
  ...
  ret =  aci_gatt_add_char(chatServHandle, UUID_TYPE_128, &char_uuid, 20, CHAR_PROP_WRITE|CHAR_PROP_WRITE_WITHOUT_RESP, (ATTR_PERMISSION_ENCRY_READ|ATTR_PERMISSION_AUTHEN_READ), GATT_NOTIFY_ATTRIBUTE_WRITE,
                16, 1, &RXCharHandle);
  if (ret != BLE_STATUS_SUCCESS) goto fail;
